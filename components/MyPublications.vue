<template>
   <div class="my-publications">
      <div class="my-publications__title">Мои объявления</div>
      <div class="my-publications__switcher">
         <div v-for="(item, index) in SWITCHER_ITEMS" :key="item" class="my-publications__item"
            :class="{ 'my-publications__item--active': selectedItem === item }" @click="handleSwitch(item, index)">
            {{ item }}
         </div>
         <div class="my-publications__indicator" :style="indicatorStyle"></div>
      </div>
      <MyPublicationsList :adsMain="adsMain" :xCountOnPage="xCountOnPage" @updateSort="handleSortChange" @refreshAds="fetchAds"
