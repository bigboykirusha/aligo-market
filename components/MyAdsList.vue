<template>
   <div class="cards">
      ...
      <div class="cards__main">
         <MyAdsCardSkeleton v-if="isLoading" v-for="index in xCountOnPage" :key="index" />
         <MyAdsCard v-if="!isLoading && adsMain.length > 0" v-for="ad in adsMain" :key="ad.id"
            :pageType="props.pageType" v-bind="mapAdProps(ad)" @updateData="handleUpdateData"
            @deleteAd="handleDeleteAd" />
         <!-- Плейсхолдер, если объявлений нет -->
...
const confirmArchive = async () => {
   await handleAction('deleteAd', 'Переместить в архив');
};

const confirmUnpublish = async () => {
   await handleAction('takeOffPublication', 'Снять с публикации');
};
...
const handleAction = async (action, message) => {
   if (selectedCount.value > 0) {
      try {
         ...
         alert(`Ошибка при ${message.toLowerCase()}. Пожалуйста, попробуйте снова.`);
      }
   } else {
